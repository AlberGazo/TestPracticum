...
class Functions(Inicializar):
    ...
    def _name_element(self, name):
        try:
            ...
            elements = self.driver.find_element(By.ID,name)
            print(u"Esperar_Elemento: Se visualizo el elemento " + name)
            return elements
    ...
    def click(self,name):
        driver = self.driver
        elemento_clic = driver.find_element(By.XPATH, name)
        elemento_clic.click()
        print("Se dio click a" + name)

    def comparartitulo(self, Nombre):
        driver = self.driver
        self.pagina = driver.current_url
        print(self.pagina)
        self.titulo = self.pagina.title
        print(self.titulo)
        self.assertEqual(Nombre, self.titulo)
        assert "No results found." not in driver.page_source

    def seleccionartextoselect(self,Nombre,Texto):
        driver = self.driver
        drop = Select(driver.find_element(By.XPATH,Nombre))
        drop.select_by_visible_text(Texto)
    ...
    def moveraelemento(self, Elemento):
        localizador = self.driver.find_element(By.XPATH, Elemento)
        action = ActionChains(self.driver)
        action.move_to_element(localizador)
        action.perform()

    def darclicelementojavascript(self, Elemento):
        localizador = self.driver.find_element(By.XPATH, Elemento)
        self.driver.execute_script("arguments[0].click();", localizador)

    def hacerscroll(self, Elemento):
        driver = self.driver
        localizador = driver.find_element(By.XPATH, Elemento)
        driver.execute_script("arguments[0].scrollIntoView();", localizador)

    def ingresardatos(self, elemento, Texto):
        driver = self.driver
        elemento_texto = driver.find_element(By.XPATH, elemento)
        elemento_texto.click()
        elemento_texto.send_keys(Texto)
        print("Se ingreso el dato")
    ...
    def obtenervariablesjson(self, entity):
            ...
            try:
                ...
                if self.json_GetFieldBy.lower() == "xpath":
                    if MyTextElement is not None:
                        ...
                        print("el elemento es " + self.json_ValueToFind)
    ...
    def page_has_loaded(self):
        driver = self.driver
        print("Checando si la pagina esta cargando".format(self.driver.current_url))
        page_state = driver.execute_script('return document.readyState;')
        yield
        WebDriverWait(driver,30).until(lambda driver: page_state == 'complete')
        assert page_state == 'complete', "No se completo la carga de la pagina"
    ...
    def comparartexto(self, locator, TEXTO):
    ...
    def textDateEnvironmentReplace(self, text):
        ...
        # Excel
    def leer_celda(self, celda,nombre):
        wb = openpyxl.load_workbook(Inicializar.Excel + "/" + nombre +".xlsx")
        sheet = wb["DataTest"]
        valor = str(sheet[celda].value)
        print(u"------------------------------------")
        print(u"El libro de excel utilizado es de es: " + Inicializar.Excel + "/" + nombre +".xlsx")
    ...
    def seleccionar_productos(self, elementotr, elementotd):
        print("Entro")
        rows = len(self.driver.find_elements(By.XPATH, elementotr))

        cells = len(self.driver.find_elements(By.XPATH, elementotd))

        print(rows)
        print(cells)
        # print("filas=%s columnas=%s" % (row_count, column_count))
        for n in range(2, rows+1):
            for b in range(1,cells+1):
                dato = self.driver.find_element(By.XPATH,"asdas")